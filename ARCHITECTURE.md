...
The **capability model** lives in `mission/Platforms.js` and is selected by the operator rather than sniffed from the link: SYS CONFIG → FLIGHT STACK says which firmware this GCS is set up for, and that decides the suggested link protocol, which mission features are offered, and whether the Flight Plan page exists at all. The route card warns when the selected stack and the connected link disagree about the protocol.

> **Known gap.** The gating covers mission planning only. With an MSP link connected the parameter page and the ARM / mode buttons still send MAVLink, which fails harmlessly but is not greyed out — they should read the same capability table.
